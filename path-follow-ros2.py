...
class YoLov5TRT(object):
    ...
    def infer(self, input_image_path):
        ...
        if torch.tensor(0) in result_classid:
            for i in range(len(result_boxes)):
                box = result_boxes[i]
                if int(result_classid[i]) == 0 and result_scores[i] >= 0.5:
                    int_box = list(map(int, box.numpy().tolist()))  # tensor变为int
...
class PublishThread(threading.Thread):
    ...
    def wait_for_subscribers(self):
        ...
        while rclpy.ok() and self.publisher.get_subscription_count() == 0:
            if i == 4:
                self.node.get_logger().info("Waiting for subscriber to connect to cmd_vel")
            time.sleep(0.5)
            i += 1
            i = i % 5
        if not rclpy.ok():
...
class YOLOv5Node(Node):
    def __init__(self):
        ...
        try:
            ...
            repeat = self.get_parameter('repeat_rate').get_parameter_value().double_value
    ...
    def process_frame(self):
        ...
        self.pub_thread.update(x, y, z, th, 
